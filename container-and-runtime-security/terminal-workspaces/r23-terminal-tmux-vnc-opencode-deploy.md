---
okf_version: "1.0"
id: "okf-con-ter-r23-terminal-tmux-vnc-opencode-deploy"
...
tags:
  ...
  - terminal-workspaces
summary: "Braucht OpenMesh einen simplen 'nur Terminal'-Deployment-Typ, ist 'OpenCode' schon"
---

# R23 — Terminal-Only Deployment, OpenCode, tmux/VNC

## Question
Braucht OpenMesh einen simplen "nur Terminal"-Deployment-Typ, ist "OpenCode" schon
...
openmesh`. There is no `terminal` (or `ttyd`/`gotty`) case anywhere in the
...
→ `dsh`/`dsh_code_server` → `opencode`/`opencode_code_server` → `openmesh`)
...
`docs/backlog/BACKLOG.md:120,164,328` records this as a known gap: `ttyd
...
Sprint 25's xterm.js work (`docs/sprints/SPRINT-25-RICH-OUTPUT-RENDERING.md:14`)
...
with provider "addons" (Claude Code, Aider, openmesh-mesh auto-provision), a
...
listed only under "Research Tasks" checkboxes (unchecked) — there is **no
...
- **Not implemented.** `deployment_schemas`/`deployment_addons` tables don't
  ...
  `docs/sprints/SPRINT-11-PROJECTS-DASHBOARD.md:50` as an open checkbox
  `[ ]`).
- **Superseded in part.** Two later research-audit passes
  ...
  already on record — it's mirrored in `docs/backlog/BACKLOG.md:120,328` as
...
### TASK-005 (`docs/tasks/TASK-005-openmesh-review-trading-agent-infra.md`)

Status: ✅ done (6.9.2026), feeds `R20-openmesh-reuse-review.md`. It is a
structural review of `saits/st-2` and `openmesh-crew` (compose topology,
...
(§5 Legacy-Schulden, line 57): *"VNC 6090–6096 without Auth — Sprint-19-Erbe;
OpenMesh-noVNC hat dasselbe Problem (WP0-KRITISCH) — Muster not übernehmen,
Fix läuft in Sprint 19/20."* — i.e. TASK-005 already flagged that
`openmesh-crew`'s per-agent VNC ports have no auth, called out that OpenMesh's
own noVNC has the identical gap, and said explicitly **not** to copy that
pattern. It did not go one level deeper into *how* openmesh-crew's VNC
...
## 4. Findings from saits-crew / openmesh-crew (sibling repos)
...
`${HOME}/projects/openmesh-mirror/openmesh-crew/deploy/claude/` (built by
...
```
This **is the fix** for OpenMesh's own `TODO(security, noVNC)` in
`projects_ssh.go:988-998`: x11vnc supports a real password
(`-passwd`/`-rfbauth`) and this script hard-fails rather than starting
unauuthenticated. TASK-005's finding that "VNC 6090-6096 without Auth" is about
...
openmesh-crew tmux+VNC pattern as the proven mechanism, while fixing the
...
2. **openmesh-crew's Xvfb+fluxbox+xterm+tmux+x11vnc+websockify stack** (§4) —
   heavier, but battle-tested, and gives tmux detach/reattach +
   `send-keys` programmatic injection (useful if OpenMesh ever wants an agent
...
- Add a new `docker run` branch alongside the existing
  `dsh`/`opencode`/`openmesh` cases (~line 982-1029) for `"terminal"`: pull a
...
- Update `frameworkStartsWorkspace()` (line 182) to return `true` for the new
  ...
  a multi-container stack like `openmesh`.
