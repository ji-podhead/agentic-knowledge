---
okf_version: "1.0"
id: "okf-con-san-r29-gvisor-ssh-networking"
...
## 1. Executive Summary
The user requested a massive architectural change to resolve ongoing Docker networking isolation issues (where workspace containers could not reach the `openmesh` gateway) and SSH authentication issues (where the backend required direct access to private key files).
...
*   `network_mode`: `isolated` (creates a dedicated bridge), `openmesh-net` (attaches to the gateway's network), `none` (total isolation).
...
**Changes to `docker-compose.yml` (`openmesh` service):**
...
1.  **Network Attachment:**
    *   If `network_mode == "openmesh-net"`, append `--network openmesh_openmesh-net` to the `docker run` command.
...
4.  **Universal OpenMesh Access:** The `openmesh` gateway container stays securely on `openmesh-net`. It will act as the single proxy/gateway for all projects attached to this network.
...
*   **Controls:**
    *   Dropdown: Network Mode (Isolated Bridge, OpenMesh Shared Network, Airgapped/None).
