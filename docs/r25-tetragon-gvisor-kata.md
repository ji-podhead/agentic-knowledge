...
date: 2026-09-08
status: final
tags: [gvisor, ebpf, kubernetes, k8s, sast]
license: CC-BY-4.0
---

# R25 — Tetragon + gVisor / Kata Containers: Verifizierung & Einordnung für SPRINT-28
...
* eBPF-basierte Security-Observability **und** Runtime-Enforcement (in-kernel
  Filter + Aktionen bis SIGKILL). TracingPolicies als YAML — funktionieren auch
  **ohne Kubernetes** via `--tracing-policy`-Flags bzw. `tetra` gRPC-CLI
...
* Einordnung gegen unsere Frage (Pre-Commit-Guard): Tetragon triggert
  ...
  (BPF-LSM-Enforcement à la AntCWPP) ist nicht der Kern; Enforcement läuft
  über Hook-Aktionen (Sigkill/Override), nicht über `security_*`-LSM-Programme.
* **Aber:** K-LAF §8b bleibt korrekt — Tetragon ist die fertige
  Beobachtungs-/Policy-Maschine, wenn wir nicht rohes `cilium/ebpf` bauen
  wollen. Entscheidung bleibt E-S2 (SPRINT-28), jetzt mit besserer Datenlage.
...
* gVisor hat eingebaute **Runtime Monitoring**-Trace-Points (alle Syscalls +
  Container-Events), gestreamt über einen **UDS/Remote-Sink** (protobuf) an
  einen externen Prozess — offiziell dokumentiert:
  https://gvisor.dev/docs/user_guide/runtimemonitoring/ und
  https://gvisor.dev/docs/tutorials/falco/ (Falco-Integration, seit ~0.33/2022).
* Das Sentry-User-Kernel-Modell stimmt: 1 Sentry-Prozess pro Sandbox; aus
  Host-Kernel-Sicht ist die Sandbox **ein Prozess**. Host-eBPF (Tetragon & Co.)
  sieht deshalb nur die Syscalls des Sentry — grob korreliert, ohne
...
* Muster: Node-Security-Agent lädt eBPF-Programme **in den Kernel der
  ...
  **LSM-Hooks im Gast-Kernel** (`security_inode_permission`, inode→Policy-Map)
  plus execve-Tracepoints, Netz- und FIM-Kontrolle.
* Relevanz für uns: Das ist der Beweis, dass „Pre-Commit-Guard **in** der
  isolierten Umgebung" nicht nur Theorie ist — aber unter einer
  Kata-ähnlichen VM-Kernel-Architektur, nicht unter gVisor.

## 2. Korrekturen am Operator-Quelltext (wichtig)

* ❌ **„gVisor pausiert den Container, bis das Host-Tool freigibt"** — nicht
  ...
  Block-until-Verdict-Fluss über den UDS-Sink existiert in der Dokumentation
  nicht. Enforcement *innerhalb* von gVisor müsste gVisor-eigene Mechanismen
  nutzen (nicht eBPF, nicht externes Verdict). Für SPRINT-28 heißt das:
  **T2 (gVisor) hat keinen echten Kernel-Pre-Commit-Block** — dort gilt
  Variante (c) async remediation (Event → Worktree-Reset + Freeze-Leiter) oder
  die Hooks-Fallback-Stufe (WP4).
* ❌ **„gVisor + Tetragon" als fertige Kombination mit Blocking** — keine
  offizielle Integration gefunden. Tetragon sieht auf dem Host nur den
  Sentry-Prozess (siehe oben). Die Vergleichstabelle im Quelltext ist
  Marketing-nah, nicht Dokumentations-nah.
* ⚠️ **Kata braucht Nested Virtualization** in Cloud-VMs — steht schon in
  K-LAF §5. Zusätzlich entschied K-LAF für T3 **KVM/libvirt statt eines
  zweiten Container-Runtime-Stacks**. Kata wäre ein *alternativer* T3-Runtime-Pfad
  mit dem AntCWPP-Vorteil (eigener Gast-Kernel → eBPF im Gast), kein Ersatz
...
|---|---|---|---|
| T1 rootless Docker | Host-Kernel, kein BPF-LSM garantiert | ❌ nur Best-Effort | WP4-Fallback (globale Git-Hooks) |
| T1+ Host mit BPF-LSM | Host-Kernel, LSM verfügbar | ✅ echtes Kernel-Blocking | WP1: eBPF-LSM am Host-Kernel (cilium/ebpf oder Tetragon, E-S2) |
| T2 gVisor | Sandbox ohne echten Kernel; Host sieht nur Sentry | ⚠️ kein Sync-Block möglich | seccheck-Trace-Points → eigener Sink (Beobachtung) + async Remediation (Freeze-Leiter) |
...
**Ein echtes „eBPF + gVisor + Kata" Alles-Könner gibt es nicht — pro Tier ist
der Mechanismus ein anderer.** Das ist selbst Artikelstoff: dieselbe Garantie
(„kein Secret landet im Commit") wird pro Isolationsstufe mit einem anderen
Werkzeug erzwungen, und nur die Identitätskette macht die Resultate
vergleichbar.
