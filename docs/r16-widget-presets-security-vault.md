...
## Vision
JiMesh dashboard wird widget-based (wie leobot2 trading bot):
- Widget Registry mit Kategorien (mesh, agents, discovery, security, system)
- Dashboard Presets (vorgefertigte grid layouts)
- App Kategorien als pseudo-layer für sidebar (deployments gruppieren)
...
### Widget Registry (JiMesh-specific)
```typescript
WIDGET_REGISTRY = {
  // mesh
...
### Smart Suggestions
When a database is deployed, JiMesh suggests complementary apps:
...
- Wireframe view: shows request flow: client → JiMesh → model → response
...
- Nodes: JiMesh proxy, chains, models, deployments, discovered services
...
- Recommendations: "Enable encryption", "Set JIMESH_ENCRYPTION_KEY"
...
### Implementation
File: `src/backend/internal/metadata/ontology.go`
