...
date: 2026-09-06
status: final
tags: [mcp, docker, mesh, ssh, redis]
license: CC-BY-4.0
---
...
### How it works
```
AI Agent (DSH, Claude Code, JiMesh agent)
...
Docker MCP Gateway
  ...
  └── MCP Server: jimesh         (our own — expose JiMesh API)
```

### Key advantages for JiMesh
...
- **Auto-registration**: JiMesh network discovery finds running MCP servers → auto-register as agent tools
- **User installs**: user runs `docker mcp server add <name>` → JiMesh detects → exposes to agents
...
| **`jimesh`** (ours) | mesh routing | deploy, provision, chains, experts |

---

## JiMesh + Docker MCP Integration Architecture

### 1. JiMesh as MCP Server
JiMesh exposes its own API as MCP tools:
- `jimesh.deploy(project, type, path)` — E2E deployment
- `jimesh.provision(project, chainId)` — auto-provision mesh endpoints
- `jimesh.list_chains()` — list mesh routers
- `jimesh.list_experts()` — list registered experts
- `jimesh.execute_experts(task)` — run MoE pipeline
- `jimesh.orchestrate(task)` — orchestration endpoint
- `jimesh.enrich_model(modelId)` — enrichment pipeline
- `jimesh.get_analytics()` — request analytics
- `jimesh.create_environment(template)` — container from template

### 2. JiMesh as MCP Client
JiMesh utility agent can use other MCP servers:
...
```
JiMesh SSH → docker mcp server list
...
2. JiMesh network discovery detects new MCP server
3. JiMesh auto-registers `postgres.query` as agent tool
...
server := mcp.NewServer("jimesh", "1.0.0")
...
// Run as Docker container: docker mcp server add jimesh
```
...
### Dockerfile for JiMesh MCP Server
```dockerfile
FROM golang:1.22 AS builder
COPY . /src
WORKDIR /src
RUN CGO_ENABLED=0 go build -o /jimesh-mcp ./cmd/mcp-server

FROM alpine:3.20
COPY --from=builder /jimesh-mcp /jimesh-mcp
ENTRYPOINT ["/jimesh-mcp"]
```
...
services:
  ...
  jimesh-mcp:
    ...
    environment:
      - JIMESH_URL=http://jimesh:9091
      - JIMESH_API_KEY=${JIMESH_API_KEY}
    depends_on:
      - mcp-gateway
```
...
### Discovery Flow
```
1. JiMesh SSH → docker ps --format json
   → parse: [container1, container2, ...]
   → for each container: check if MCP server (labels: mcp=true)

2. JiMesh SSH → docker mcp server list --format json
...
JiMesh knows these connector types. Each can be auto-discovered or manually added:

| Connector | Detection Method | MCP Server | Agent Tools |
|---|---|---|---|
| DSH | port 3079, docker label | `jimesh` (our MCP) | deploy, provision, orchestrate |
...
| JiMesh | (self) | `jimesh` MCP server | all JiMesh API as tools |

---

Hier ist die vollständige Systemspezifikation und das Architektur-Design für das Modul R14 — Docker MCP Integration. Dieses Modul nutzt das native Docker Model Context Protocol (MCP) Toolkit, um verifizierte, isolierte Agenten-Werkzeuge nahtlos in den JiMesh Go Core und die MoE-Inferenz-Pipeline zu integrieren [2601.13671].1. Docker MCP Gateway & Network Discovery ArchitectureDurch die Integration des Docker MCP Gateways entfällt das manuelle Verwalten einzelner Werkzeug-Sockets. Der JiMesh Go Core fungiert synchron als MCP Client (konsumiert verifizierte Konnektoren) und als MCP Server (stellt JiMesh Core-Features via JSON-RPC für externe Agenten wie Claude Code bereit) [2601.13671].┌─────────────────────────────────────────────────────────────────────────┐
...
└────────────┬───────────────────────┬───────────────────────┬────────────┘
   ...
   ┌───────────────────┐   ┌───────────────────┐   ┌───────────────────┐
   │ mcp/filesystem    │   │ mcp/postgres      │   │ mcp/jimesh        │
   │ (Host Path Mount) │   │ (DB Auth Connect) │   │ (Our Go Binary)   │
   └───────────────────┘   └───────────────────┘   └───────────────────┘
🔄 Autonome MCP Discovery-Pipeline:Das JiMesh-Hintergrund-Parsing führt periodisch über den SSH-Kanal (R11) den Befehl docker mcp server list --format json aus.Erkannte Konnektoren (z. B. postgres, github) werden ausgelesen und im Dashboard-Tab "Discovery" mit einem exklusiven MCP Blue-Badge versehen.Der Go-Core fragt über das Gateway die verfügbaren Werkzeug-Signaturen ab und injiziert sie direkt in die Function-Calling-Ebene des utility-Modells.2. Implementierung: Go MCP Server & Client CoreUnter Verwendung von github.com/mark3labs/mcp-go implementiert JiMesh die bidirektionale Registrierung nativ [2601.13671].A. JiMesh als MCP Server (Bereitstellung unserer API für Agenten)gopackage mcp_server
...
func InitJiMeshMCPServer() *server.MCPServer {
	s := server.NewMCPServer("jimesh", "1.0.0")
...
B. JiMesh als MCP Client (Abfrage und Steuerung des Docker Gateways)gopackage mcp_client
...
3. Docker-Infrastruktur-Code für das GatewayDas System nutzt dedizierte Container, um Isolation und Secret-Management kryptografisch vom Host-System zu trennen.Dockerfile für unseren JiMesh MCP Server (cmd/mcp-server/Dockerfile):dockerfileFROM golang:1.22-alpine AS builder
WORKDIR /app
COPY go.mod go.sum ./
RUN go mod download
COPY . .
RUN CGO_ENABLED=0 GOOS=linux go build -o /jimesh-mcp-engine ./cmd/mcp-server/main.go

FROM alpine:3.20
RUN apt-get update && apt-get install -y ca-certificates
COPY --from=builder /jimesh-mcp-engine /jimesh-mcp-engine
EXPOSE 8765
ENTRYPOINT ["/jimesh-mcp-engine"]
Produktionsbereites Docker Compose mit Secret-Mounts (docker-compose.mcp.yml):yamlservices:
  mcp-gateway:
    image: docker/mcp-gateway:latest
    container_name: jimesh-mcp-gateway
    restart: unless-stopped
    volumes:
      - /var/run/docker.sock:/var/run/docker.sock
      - jimesh-mcp-secrets:/secrets:ro # Isolierter Lesezugriff für OAuth/API-Keys
    ports:
      - "8765:8765"
  ...
  jimesh-mcp:
    ...
    container_name: jimesh-mcp-server
    environment:
      - JIMESH_CORE_URL=http://jimesh-core:9091
      - JIMESH_API_KEY=${JIMESH_API_KEY}
    depends_on:
      - mcp-gateway
    networks:
...
volumes:
  jimesh-mcp-secrets:
    external: true
4. JSON-Schema Spezifikation der Core-SchnittstellenDiese Deklarationen werden vom Docker-Gateway serialisiert und an angebundene LLMs zur Funktionsaufruf-Generierung ausgesendet.jimesh.provisionjson{
...
5. Aktualisierter Projekt-Backlog & EpicsWir fügen Epic 5 hinzu, um die Integration und Validierung der Docker MCP Connectors nahtlos abzubilden.🏛️ Epic 5: Docker MCP Architecture & Sandboxing (R14)Sprint 8: Gateway Setup & Discovery ImplementationTASK-5.1 Integration des docker mcp CLI-Parsers in die SSH-Auto-Discovery-Engine des Go-Cores.TASK-5.2 Implementierung des JSON-RPC-Client-Protokolls zur Anbindung an den UNIX-Socket des Docker MCP Gateways.TASK-5.3 Erstellung der UI-Ansichten für das Discovery-Konnektoren-Grid mit integrierten Steuerelementen zur Aktivierung/Deaktivierung spezifischer Werkzeuge.Sprint 9: Secure Tool Injection & MoE UpgradeTASK-5.4 Implementierung der Go-Server-Routen für jimesh.deploy und jimesh.execute_experts innerhalb der mcp-go Bibliothek.TASK-5.5 Erweiterung des Hierarchical Planner Agenten (R12), sodass dieser bei Daten-Operationen transparent auf den verifizierten postgres-Konnektor zugreifen kann, anstatt rohe Terminal-Befehle auszuführen.Möchtest du als Nächstes die vollständige Go-Logik für den JSON-RPC Stream-Parser (TASK-5.2) sehen, der Chunks über das Docker MCP Gateway austauscht, oder sollen wir das React-Frontend-Formular für den neuen MCP-Katalog-Tab entwerfen?