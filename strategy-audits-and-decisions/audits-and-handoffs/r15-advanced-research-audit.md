---
okf_version: "1.0"
id: "okf-str-aud-r15-advanced-research-audit"
...
tags:
  ...
  - audits-and-handoffs
summary: "Full audit of R1-R14. Cuts overkill, recommends open-source shortcuts,"
---
...
- **Replace with**: MCP tools (openmesh.deploy, openmesh.provision via structured JSON-RPC).
...
- **Replace with**: Docker Events API stream (cli.Events(ctx, ...)). Async,
  0% CPU idle. Container starts → OpenMesh catches event → reads labels → registers.
...
6. **MCP Server (mcp-go)** — expose OpenMesh API as structured tools
