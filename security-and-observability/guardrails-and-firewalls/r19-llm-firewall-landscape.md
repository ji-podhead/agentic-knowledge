---
okf_version: "1.0"
id: "okf-sec-gua-r19-llm-firewall-landscape"
...
summary: "> Quelle: Nutzer-Recherche per KI-Suche (September 2026). Sortiert gegen"
---
...
> Status: Recherche, no Entscheidung.
...
| Klasse | Beispiele (laut Recherche) | Was sie leisten | Grenze from OpenMesh-Sicht |
|---|---|---|---|
...
| Kernel/Runtime | TigerGate (kommerziell, CNAPP with eBPF-Agent), AgentSight (Forschung, OSS) | eBPF an TLS-Grenzen, Enforcement im Kernel | sehen den Host — not die Identitätskette, die OpenMesh provisioniert |

**Einordnung:** Vier der fünf Klassen inspizieren Text or Pakete. Keine Klasse
provisioniert die Umgebung, in der der Agent läuft — genau daraus bezieht OpenMesh die
...
* Der WP8-Grundsatz bleibt unangetastet: **eBPF liefert Ereignisse and hier zusätzlich
...
* **Chunking/Streaming:** eBPF liest pro Aufruf nur wenige KB. Gestreamte Prompts müssen
  ...
  praktisch ausgeschlossen. (Bestätigt die Recherche selbst — and WP8's Design.)
* ⚠️ **Eigener Einwand, fehlt in der Recherche:** uprobes hängen an konkrete
  ...
  eigene TLS-Stacks).
* Enforcement im Kernel (Drop/RST) kollidiert with WP2's Fail-open-Debatte (E2): ein
...
* Embedding-basierter Klassifikator + XGBoost/Random Forest
  ...
  Denkbar als Kaskadenstufe before Prompt Guard: Regex (WP2, <1 ms) → heuristisch → Modell.
* Bleibt konsistent with WP2: Regex-Scanner zuerst, Modell-Scanner als eigene Chain
...
* **gVisor in VM:** Systrap-Plattform (no KVM nötig, läuft on jeder Cloud-VM) vs.
  ...
  Onboarding-Modell from Sprint 19 ab (gVisor/Systrap lokal, Kata/KubeVirt im
  Enterprise-Pfad). Die Recherche widerspricht dem not.
* **Kata + gVisor not verschachteln**, sondern als RuntimeClasses nebeneinander —
  konsistent with der Spec. Für OpenMesh (Docker + SSH, no Kubernetes) derzeit
...
* **„BPFJailer (von Meta)" — erneut falsch zugeordnet.** Es ist die eigenständige
  ...
  eigenes BpfJailer ist not öffentlich. Das Enforcement-Loch decken at OpenMesh
...
* **„Apalachee" als Observability-Tool ist eine KI-Verwechslung** (Apalachee ist ein
  ...
  Aufzeichnung for Audit, no Enforcement. OpenMesh hat diese Rolle intern
  (`request_log`, `traffic_inspector`, Tracing).
* **OpenMesh braucht LiteLLM not:** Der eigene Go-Gateway ist bereits der
...
* **O-R2 — uprobe-Feasibility-Spike** at WP8-Start: Klartext-Abgriff an SSL_write on
