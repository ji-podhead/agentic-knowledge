---
okf_version: "1.0"
id: "okf-sec-gua-r40-misalignment-audit"
...
summary: "**Quelle:** Operator 14.9. („alles kacke … ich wollte das als ganzes anders,"
---
...
1. OpenMesh = sichere Environment for Vibe-Coding + Agents (SaaS, self-hosted)
...
### M3 — Vault: EIN globaler KV-Store, no Project/Workspace-Trennung ✅ BEHOBEN (TASK-055, 15.9.)
...
- **Soll (Operator: „nur ein Vault KV — not geklärt"):** Secrets-Trennung
  after Project (mindestens) with Vault-ACL je Workspace; Spalte `project_id`
  + Row-Filter; service-keys lesen nur ihr Project
- **Entscheidung (umgesetzt):** Spalte + WHERE-Filter — no Schema-Bruch,
  ...
  - Migration: `vault_secrets` + `vault_audit_log` um `project_id`/
    ...
    Legacy-Zeilen behalten `''` and bleiben Operator-erreichbar (Backward-compat)
  ...
  - Endpoint-Layer: Scope from authentifizierter Identität — Service-Key →
    ...
    `?project=` or alle (Operator). NIE from dem Body (X-Header-Kontrakt)
  - Tests: `store/vault_test.go` (Projekt A sieht B not — openmesh_test-DB),
...
### M4 — RBAC: Rollen existieren nur als Budget-Spalte, no Zuweisungs-UI ✅ FIXED (TASK-056)
...
- **FIX (TASK-056, 15.9.):** AdminUsersPage.tsx — jede User-Zeile aufklappbar →
  Rollen-Picker pro (User, Project) about POST /api/budgets (rbac.go schreibt
  ...
  zeigt ALLE Zuweisungen (GET /api/budgets without projectId) with inline
...
### M6 — Firewall-Rules-UI: nur Status-Tile, no Editor ❌
...
- **Fund:** containers.workspace_id existiert (live, 037), aber no View
...
- **Fund:** Preset-Picker zeigt „gVisor-NAT" without Kontext; Operator-Feedback
...
- **Soll:** Nach 053-Merge: ProjectsPage on Deploy-Kern reduzieren,
...
| AIM-Feature | Haben wir? | Bewertung for OpenMesh |
|---|---|---|
...
| 5-Schritt-FGA (Capability→Attribute→Context→Chain→Intent) | Nein (OPA 1-Step) | Overkill for v1; Intent-Check (NanoMind 3M lokal) ist aber ein schönes Muster for K-LAF WP2-Variante |
| 9-Faktor-Trust-Scoring | Nein | R21/DAP-Skill-Score-Vorlage passt dazu — später |
| **JIT-Access with Human-Approval** (Pause bis Dashboard-Freigabe) | **NEIN — aber R21/DAP GUARDED-Approval-Queue ist genau das!** | Muster übernehmen for Epic 18 |
...
Input for Epic-3-WP3 (Tool-Call-Gating for Subagents/DB/MCP) übernehmen —
+ JIT-Approval-Muster als Implementierungs-Vorlage for die GUARDED-Queue
(R21 §6, Epic 18). In BACKLOG: Epic-3-WP3-Zeile with AIM-Referenz ergänzen.

---

## R40-Appendix B — Quick-Deploy-Modal (Operator-Idee, 14.9.)

**Operator:** „Unten in der Sidebar nur ein Plus-Icon for Deploy-Zeug — das
macht ein Modal on. Wenn man deployed and Workspace or SSH-Target not
da ist, kann man das GLEICH MIT-ERSTELLEN or Preset wählen."

**Konzept (ergänzt 053 — hebt die Trennung NICHT on):**
- Sidebar unten: [+] → Quick-Deploy-Modal
- Modal-Flow: App wählen/erstellen → Workspace wählen ODER inline-anlegen
  (falls no da) → SSH-Target wählen ODER inline-onboarding (falls keines
  da) → Preset wählen → Deploy
- Die getrennten Verwaltungs-Sichten (Endpoints-Page, Workspaces-Page)
  bleiben — das Modal ist der Convenience-Einstieg, der at fehlenden
  Entitäten inline zum Erstellen anbietet
- Implementation: after 053-Merge als Ergänzung (gleiche Komponenten, die
