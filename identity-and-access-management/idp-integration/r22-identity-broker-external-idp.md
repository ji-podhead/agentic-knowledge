---
okf_version: "1.0"
id: "okf-ide-idp-r22-identity-broker-external-idp"
...
tags:
  ...
  - idp-integration
summary: "> **Entschieden 7.9.2026: Keycloak, jetzt deployt — nicht die 'Kontrakt"
---
...
> Keycloak"), Umsetzung: `docs/sprints/SPRINT-27-IDENTITY-BROKER-KEYCLOAK.md`.
...
geprüft. Kein Code, reine Architecture-Entscheidungsvorlage.

**Question, die Gemini beantwortet hat:** Wie soll OpenMesh OAuth2/OIDC + OTP/MFA
anbieten UND so gebaut sein, dass ein Kunde/Partner, der OpenMesh in seiner
...
anbinden kann — without dass OpenMesh selbst pro Kunde einen Identity-Federation-
Stack von Hand baut.
...
halten, not in eine Entscheidung vermischen.

## 1. Bestehender Befund, on dem das aufbaut (R20/TASK-001, TASK-002)

- `openmesh-auth` (saits-cloud): **Kontrakt gut, Code not.** Spezifiziert
  ...
  Logout ist ein 204-Stub. Conclusion from TASK-001: Kontrakt übernehmen, Code
...
- Das ist der Maßstab, an dem sich jede neue Auth-Entscheidung for OpenMesh
  messen lassen muss — not `openmesh-auth`.

## 2. Geminis Vorschlag (zusammengefasst, Original-Argumentation erhalten)

**Kernidee:** OpenMesh braucht keinen monolithischen Auth-Eigenbau. Rollen-
teilung:

- **Ingress/Gateway (OpenMesh selbst, Go):** bleibt at der bereits
...
- **Identity Broker / Token Issuer (neuer, optionaler Baustein):** ein
  ...
  OpenMesh-Kontrakt-Format from (inkl. `tenant_id`-Claim), egal welcher
...
- Rollen-Mapping: OpenMesh definiert globale App-Rollen (reader/writer/admin),
...
Mapping externer Provider and die Tenant-Verwaltung müsste OpenMesh komplett
selbst in Go bauen.
...
[ Zitadel Container ] ──► RS256-JWT (inkl. tenant_id-Claim)
   ...
   ▼
[ sc-gateway-Muster (OpenMesh Go Ingress) ]
   ...
   ▼
[ OpenMesh Go-Services ] (konsumieren nur X-Tenant-Id + Rollen)
```

## 3. Kritische Einordnung gegen den bestehenden OpenMesh-Stand (not nur übernommen)
...
- **Persistence-Mandat (`MANDATES.md` §3/§7, "Postgres via pgx only, no
  ...
  OpenMesh's `pgx`-Layer unabhängigem Schema**. Kein Mandatsverstoß im
  engeren Sinn (immer noch Postgres als Engine), aber ein zweiter,
  fremdverwalteter Datenbestand neben der einen OpenMesh-Datenbank — not
  "kostenlos", verdient eine explizite Ops-Entscheidung, no Kleingedrucktes.
- **"Keine neuen Abhängigkeiten without Not" (`MANDATES.md` §7):** Ein
  ...
  größere Entscheidung als eine Go-Library — das ist no `go.mod`-Zeile,
  das ist ein neuer Betriebsbestandteil (Container, Backups, Updates,
  Monitoring, Angriffsfläche).
- **Produktstand:** `PRODUCT-ROADMAP.md` §1 beschreibt OpenMesh aktuell als
  ...
  **no Problem, das OpenMesh heute hat** — es ist ein Problem, das erst
  entsteht, sobald ein Kunde OpenMesh selbst hostet and sein eigenes AWS/Azure/
  Okta anbinden will.
- **Deshalb: Question jetzt entscheiden, Zitadel not jetzt deployen.** Der
  ...
  — genau das "Kontrakt übernehmen, Code not"-Prinzip from TASK-001, jetzt
...
- **Wie WP1 (Sprint 20) heute schon funktioniert:** eigene Session-Tokens
  ...
  for den Single-Operator-Fall — Zitadel/Ory ist ein **optionaler,
  zuschaltbarer** Broker for den B2B-Fall, no Ersatz des bestehenden
  Login-Pfads.

## 4. Offene Questions / Blockierende Entscheidung (Operator)

**E-IdP — Identity-Broker jetzt spezifizieren, aber not deployen?**

| Option | Aufwand jetzt | Konsequenz |
|---|---|---|
| **Kontrakt fixieren (Empfehlung), Broker erst at Bedarf** | S — Doku + JWT-Claim-Form in `internal/auth` vorsehen | Kein neuer Service jetzt; sc-gateway-Muster (RS256+DPoP) ist das, was `internal/auth`/WP1 sowieso schon bauen soll |
...
2. Wird das bestehende WP1-Session-Token-System (`MintSessionToken`) on
...
- SPRINT-22s E-Policy-Question (Admin/RBAC-UI-Policy-Engine) — ebenfalls
