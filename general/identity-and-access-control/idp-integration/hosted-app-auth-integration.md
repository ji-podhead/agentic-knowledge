---
okf_version: "1.0"
id: "okf-ide-idp-hosted-app-auth-integration"
...
tags:
  ...
  - idp-integration
summary: "**Source:** System Administrator-Frage 14.9.2026 ('wir wollen als superadmin auch direkt"
---

# R42 — Superadmin Visibility into a Hosted App's Own Auth/RBAC: Design Synthesis

**Source:** System Administrator-Frage 14.9.2026 ("wir wollen als superadmin auch direkt
die auths and datenbanken and rbac der apps verwalten... das selbe rbac and
die rollen in den apps sollte man dann auch direkt about llm-mesh-gateway benutzen
können"). Konkreter Anlass: der Chef des System Administrators hat Better Auth in seinen
eigenen Server integriert; der System Administrator wird diesen Server als The Multi-Provider Gateway-
...
The Multi-Provider Gateway hostet fremde Apps (SSH Remote Target → Workspace → App). Jede App
...
Entwickler gewählt hat). Der System Administrator will als Superadmin in The Multi-Provider Gateway sehen
(and teilweise verwalten) können, wer in der gehosteten App Nutzer/Rolle X
ist — without for jede App-Auth-Bibliothek eine own, tief gekoppelte
...
Der ursprüngliche Gedanke (weil The Multi-Provider Gateway's Router-Node ohnehin als DB-Proxy
before jeder App-Datenbank sitzt — Milestone Work Package): einfach die Auth-Tabellen
der App direkt about den Router lesen. Die Research (Prompt 1+2)
...
- **Cache-/Session-Desync (kritisch):** Better Auth (and die meisten
  ...
  der Nutzer bleibt bis zum TTL-Ablauf eingeloggt, obwohl The Multi-Provider Gateway ihn
  gesperrt zu haben glaubt. Ein `docker stop` on Container-Ebene würde
  das umgehen, aber genau das will der System Administrator hier not (er will
...
- **Schema-Drift:** Auch at Libraries with einem "stabilen Kern-Schema"
  ...
  Admin-Plugin fügt own Spalten hinzu) and abweichende
...
- **Krypto-/Event-Bypass:** Ein direkter `INSERT` in die `user`-Tabelle
  ...
  (Rolle ändern, Sperren) ist es real and laut Research "urgent
  abzuraten".
...
selbst gebaut). The Multi-Provider Gateway ist in der Rolle "hostet fremden, not
...
Lösung — hat aber drei Eigenschaften, die es for the gateway router's Fall (ein
...
Das ist der eigentliche Finding dieser Research (Prompt 8), and er dreht die
...
3. **the gateway router's Router-Node (der ohnehin als L7-Proxy before jedem Workspace
   sitzt, Milestone Work Package) holt/cached diesen JWKS and verifiziert das JWT
   lokal** — Signatur + `exp` prüfen, `role`-Claim extrahieren. Kein
   Netzwerk-Call zurück zu Better Auth or dessen DB pro Request.

Das ist strukturell fast identisch zu dem, was the gateway router's eigener Keycloak-
JWT-Verifier bereits tut (`internal/auth/keycloak_oidc.go`) — nur for die
Rollen einer *fremden* App statt for the gateway router's own Nutzer. Der Router
...
- **JWT ist no Ersatz for die primäre Session** — laut Doku "not meant
  ...
  nachgelagerte Dienste wie the gateway router's Router gedacht.
...
**v1-Scope-Recommendation:** read-only, opt-in pro Workspace ("dieser
...
The Multi-Provider Gateway-Dashboard, with explizit kurzer Cache-TTL. Keine Schreib-Aktionen
(Sperren/Rollen-Ändern) in v1 — das wäre der v2-Step about das
...
zuweisen) about The Multi-Provider Gateway gewünscht sind — not nur Sichtbarkeit — schlägt
die Research (Prompt 7) ein Go-Interface before, das dem bereits im Repo
...
Design-Decision: **"Rollen" not in ein starres Feld zwingen** — der
...
ein Lucia/Auth.js-Treiber gegen eine vom System Administrator konfigurierte
...
| Ory Kratos | NIE roh per SQL — own Admin-API zwingend (schema-agnostische JSON-Traits) | hoch (+ Ory Keto for Rollen) |
...
Die Research (Prompt 9) ist hier eindeutig and beruhigend, solange wir
uns an eine Grenze halten:

- The Multi-Provider Gateway bleibt **Auftragsverarbeiter (Processor/Sub-Processor)**, solange
  der Zugriff ausschließlich der vom Kunden beauftragten
  App-Verwaltung dient (dokumentierte Weisung). The Multi-Provider Gateway würde erst zum
  **gemeinsam Verantwortlichen (Joint Controller)**, wenn diese Daten for
  own Zwecke genutzt würden — z. B. plattformübergreifendes
  Benchmarking, own Nutzerprofile, or Modell-Training about
...
- Ein bestehendes Art.-28-DPA reicht not automatisch — es muss den
  ...
  The Multi-Provider Gateway dem Kunden at DSAR/Löschungsanfragen seiner Endnutzer hilft.
- **Referenz-Vorlage:** Supabase's öffentliches Data Processing Addendum
  ...
  Ausgangspunkt for eine The Multi-Provider Gateway-DPA-Ergänzung dienen, bevor dieses
  Feature real angeboten wird — auch im MVP/Pilot with dem Chef-Server.

## 7. Recommendation / nächste Steps

1. **v1 bauen:** pro Workspace optional konfigurierbar — JWKS-Endpunkt +
   Rollen-Claim-Feldname der gehosteten App; the gateway router's Router liest das
...
2. **v1 explizit NICHT bauen:** DB-Direktzugriff on App-Auth-Tabellen,
   Schreibaktionen (Sperren/Rolle ändern) about The Multi-Provider Gateway.
...
## Sources (Gemini-Research, 14.9.2026, roh in `R42_multi.md`/`R42_multi2.md`)
