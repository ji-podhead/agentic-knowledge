---
okf_version: "1.0"
id: "okf-age-bro-agent-browser-control"
title: "R12 — Agent Browser Control: Utility Model steuert The Multi-Provider Gateway Dashboard + Iframes"
...
tags:
  ...
  - browser-automation
summary: "Das utility model and die engine sollen direkt den browser kontrollieren können:"
---

# R12 — Agent Browser Control: Utility Model steuert The Multi-Provider Gateway Dashboard + Iframes
...
- [ ] **Playwright** (Go: playwright-go)
  ...
  - Can control the The Multi-Provider Gateway dashboard itself
...
- [ ] Recommendation: chromedp (Go-native, CDP, connect to existing browser)
...
- [ ] Lösung B: postMessage API
  - The Multi-Provider Gateway dashboard ↔ iframe kommunikation via window.postMessage
  - DSH plugin exposet postMessage API (wie R7 DSH plugin)
  - The Multi-Provider Gateway sendet commands → DSH plugin führt from
...
- [ ] Recommendation: Lösung A (SSH tunnel) for backend control + Lösung B (postMessage) for frontend
...
- [ ] Subagent spawning:
  - Utility model kann subagents spawnen (MoE from Milestone)
...
### 6. Integration with bestehender Architektur
- [ ] Orchestration endpoint (Milestone Work Package) erweitern:
  - `POST /api/orchestrate` with `browser_control: true`
  - Agent bekommt browser tools + API tools
  - Agent kann beide nutzen je after task
- [ ] MoE experts (Milestone) erweitern:
  - Expert type: "browser_system administrator" — kann browser steuern
  - Expert type: "api_system administrator" — kann The Multi-Provider Gateway API calls machen
  - Expert type: "coder" — kann code schreiben
- [ ] Entropy routing (Milestone Work Package):
...
- [ ] Fix optionen:
  ...
  - C: The Multi-Provider Gateway frontend: file path clicks abfangen → in The Multi-Provider Gateway editor öffnen statt xdg-open
  - D: Environment variable setzen im deployment
- [ ] Recommendation: C (frontend abfangen) + B (fallback config)
...
Utility Model (Orchestrator Agent)
  ...
  ├── API Tools (The Multi-Provider Gateway REST)
  │   ├── create deployment, provision mesh, start/stop
  │   └── list chains, add members, etc.
  ├── Subagent Spawner (MoE from Milestone)
...
Hier ist das vollständige, systemübergreifende Design for das Königsdisziplin-Modul R12 — Agent Browser Control. Dieses Modul verwandelt dein Utility-Modell in einen autonomen System Administrator, der sowohl die The Multi-Provider Gateway-API als auch das UI-Dashboard and dessen eingebettete iFrames manipulieren, koordinieren and überwachen kann.1. Browser Automation Library EmpfehlungFür ein hochperformantes Go-basiertes Ökosystem lautet die klare Recommendation: github.com/chromedp/chromedp.Begründung:Go-Native & Zero-Dependency: Im Gegensatz zu playwright-go (welches im Background Node.js-Treiber and schwere Binaries nachlädt) kommuniziert chromedp direkt about native Go-Kanäle with dem Chrome DevTools Protocol (CDP) via WebSockets.Anbindung an bestehende Sessions: chromedp kann sich an eine bereits geöffnete, headless or headed Browser-Instanz hängen (Remote Allocator). Das Utility-Modell operiert somit im exakt gleichen Context wie der menschliche User.Low-Level Frame Manipulation: CDP erlaubt das direkte Injizieren von JS-Code in isolierte iFrame-Targets, selbst wenn diese about Cross-Origin-Barrieren geschützt sind.2. Agent Control API (Function Calling Schema)Das Utility-Modell steuert den Browser deklarativ about standardisierte JSON-Schemas via Function Calling. Hier sind die drei wichtigsten Werkzeuge des System Administrators:browser_navigatejson{
  "name": "browser_navigate",
  "description": "Navigates the browser instance to a specific path within the The Multi-Provider Gateway dashboard.",
  "parameters": {
    "type": "object",
    "properties": {
...
3. Iframe Control Strategy (The Same-Origin Bridge)Da iFrames im Browser-DOM standardmäßig der Same-Origin-Policy (SOP) unterliegen, nutzen wir eine duale Architektur, um dem Agenten vollen Lese- and Schreibzugriff zu gewähren:Backend-Ebene (Der SSH-Proxy Tunnel): Wie in R11 definiert, laufen alle iFrame-Assets through den The Multi-Provider Gateway Go Core Proxy. Dadurch nimmt der Browser alle iFrames als Same-Origin (localhost:9090) wahr. chromedp kann somit without Sicherheitsblockaden direkt in den DOM-Tree der iFrames (cdp.WithTargetID) eintauchen, um Terminal-Inputs zu simulieren or Chat-Antworten zu lesen.Frontend-Ebene (Die postMessage API): Für schnelle, not-destruktive Interaktionen (z. B. "Bewege den Cursor im Editor on Zeile 10") nutzt das Dashboard eine deklarative Event-Brücke. Der Go-Core triggert ein Event im Dashboard, welches via window.postMessage an das DSH- or OpenCode-iFrame gereicht wird.4. Task Planning + Subagent Coordination DesignWenn der Haupt-Orchestrator (Utility Modell) einen komplexen Befehl erhält, erzeugt er eine hierarchische Ausführungskette (Hierarchical Agentic Workflow):                       [ High-Level User Task ]
                                  │
                                  ▼
                     ┌──────────────────────────┐
                     │ Utility Model            │
