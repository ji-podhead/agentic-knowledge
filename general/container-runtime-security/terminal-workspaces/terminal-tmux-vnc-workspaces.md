---
okf_version: "1.0"
id: "okf-con-ter-terminal-tmux-vnc-workspaces"
...
tags:
  ...
  - terminal-workspaces
summary: "Braucht The Multi-Provider Gateway einen simplen 'nur Terminal'-Deployment-Typ, ist 'OpenCode' schon"
---

# R23 — Terminal-Only Deployment, OpenCode, tmux/VNC

## Architectural Question
Braucht The Multi-Provider Gateway einen simplen "nur Terminal"-Deployment-Typ, ist "OpenCode" schon
ein echtes Angebot, and was würde tmux+VNC als Deployment-Mechanismus
bedeuten? Zusätzlich: gibt es einen Plan for "Utility-Deployment beim Anlegen
...
`store.ProjectRow.Framework` (the Backend Gateway Core:1542`) is a
...
llm-mesh-gateway`. There is no `terminal` (or `ttyd`/`gotty`) case anywhere in the
deploy dispatch switch — `frameworkStartsWorkspace()`
(the Backend Gateway Core:182-191`) and the docker-run
branch chain in `executeDeploy()` (same file, ~line 974 onward: `isRuntimePreset`
→ `dsh`/`dsh_code_server` → `opencode`/`opencode_code_server` → `llm-mesh-gateway`)
...
- the Backend Gateway Core:38` — `7681: "ttyd"` in
  `KnownPorts`, used to label an already-running service found by scanning a
  node, not to deploy one.
- the Backend Gateway Core:207-215` — `ttyd` and `gotty`
  entries under `Category: "Terminals"`, again metadata for classifying
  discovered ports/services (icon, display name).
- the Authorization Engine:358-362` — `TypeToCategory` maps both
  `"terminal"` and `"ttyd"` to `"Terminals"` for the app-category
  smart-suggestion feature (Milestone Work Package), not for the Framework deploy enum.
- the Backend Gateway Core:169` and
...
only `Framework` union in the Workspace Management View:30` is the
...
`docs/backlog/Roadmap.md:120,164,328` records this as a known gap: `ttyd
sidecar (replace custom terminal)` is listed `[ ]` (open) under Architecture Pillar, and
the packages table lists `Terminal | @xterm/xterm + ttyd sidecar | planned`.
Milestone's xterm.js work (`docs/sprints/Milestone.md:14`)
...
- `projects_ssh.go:987-1002` — the actual `docker run` for `opencode`:
  ```
<<<<<<< HEAD:knowledge/container-and-runtime-security/terminal-workspaces/r23-terminal-tmux-vnc-opencode-deploy.md
  docker run -d --name openmesh-framework-<id>-<run> -p 127.0.0.1:$ALLOC_VNC:6080 \
    -v <workspace>:/workspace <net/host env> ghcr.io/ji-workstation/opencode-vnc:latest

  docker run -d --name llm-mesh-gateway-framework-<id>-<run> -p 127.0.0.1:$ALLOC_VNC:6080 \
    -v <workspace>:/workspace <net/host env> ghcr.io/ji-podhead/opencode-vnc:latest
  ```
- `projects_ssh.go:1238-1239` — deploy-response security note: noVNC on 6080
  ...
  988-998).
- Frontend: the Workspace Management View:1239/1504` offers "OpenCode (VNC Desktop)" and
...
- `providerinfo.go:137`, the Ingress API Gateway:2738`, `quota.go:147,198` wire OpenCode
...
## 2. What Requirement / R9 already say, and implementation status
...
with provider "addons" (Claude Code, Aider, llm-mesh-gateway-mesh auto-provision), a
...
listed only under "Research Objectives" checkboxes (unchecked) — there is **no
...
- **Not implemented.** `deployment_schemas`/`deployment_addons` tables don't
  ...
  `docs/sprints/Milestone.md:50` as an open checkbox
  `[ ]`).
- **Superseded in part.** Two later research-audit passes
  ...
  already on record — it's mirrored in `docs/backlog/Roadmap.md:120,328` as
...
### Requirement (`docs/tasks/Requirement.md`)

Status: ✅ done (6.9.2026), feeds `R20-llm-mesh-gateway-reuse-review.md`. It is a
structural review of `saits/st-2` and `llm-mesh-gateway-crew` (compose topology,
...
(§5 Legacy-Schulden, line 57): *"VNC 6090–6096 without Auth — Milestone-Erbe;
The Multi-Provider Gateway-noVNC hat dasselbe Problem (Work Package-KRITISCH) — Muster not übernehmen,
Fix läuft in Milestone/20."* — i.e. Requirement already flagged that
`llm-mesh-gateway-crew`'s per-agent VNC ports have no auth, called out that The Multi-Provider Gateway's
own noVNC has the identical gap, and said explicitly **not** to copy that
pattern. It did not go one level deeper into *how* llm-mesh-gateway-crew's VNC
...
Searched:
- the Backend Gateway Core in full — the "node
  ...
  network from Requirement, which attach to the workspace container being
...
- `grep -rni "kv_storage\|kv storage\|utility kv"` across `docs/` and
  ...
  gespeichert wird and keycloak and vault auch direkt verknüpft werden" idea
...
Milestone) from "per-SSH-node utility" (doesn't exist, unclear if wanted).

---

## 4. Findings from saits-crew / llm-mesh-gateway-crew (sibling repos)

Requirement already covered compose-level structure; this section goes one
level deeper into the one thing Requirement didn't: **how the VNC+terminal
...
`${HOME}/projects/llm-mesh-gateway-mirror/llm-mesh-gateway-crew/deploy/claude/` (built by
...
```
This **is the fix** for The Multi-Provider Gateway's own `TODO(security, noVNC)` in
`projects_ssh.go:988-998`: x11vnc supports a real password
(`-passwd`/`-rfbauth`) and this script hard-fails rather than starting
unauuthenticated. Requirement's finding that "VNC 6090-6096 without Auth" is about
...
contrary to a first read of Requirement line 57. This nuance is worth folding
back into Requirement/Milestone/20 hardening notes if not already known.
...
No CDC/DB-sync pattern was found (Requirement already noted this as a negative
result) and nothing new on multi-node secrets beyond what Requirement recorded.
`${HOME}/projects/trading` was **not** re-explored — a first grep attempt
there hit a huge, irrelevant vendored `.qoder/repowiki` metadata JSON tree
(11+ MB) with no signal, and Requirement already covered `st-2`'s compose/topology
structurally; going file-by-file into `${HOME}/projects/trading` beyond that
would violate this task's own time-box and Requirement's explicit "no
...
llm-mesh-gateway-crew tmux+VNC pattern as the proven mechanism, while fixing the
...
1. **ttyd sidecar** (the standing backlog item, `Roadmap.md:120,328`) — one
...
2. **llm-mesh-gateway-crew's Xvfb+fluxbox+xterm+tmux+x11vnc+websockify stack** (§4) —
   heavier, but battle-tested, and gives tmux detach/reattach +
   `send-keys` programmatic injection (useful if The Multi-Provider Gateway ever wants an agent
...
**Provisioning/deploy changes** (the Backend Gateway Core):
- Add a new `docker run` branch alongside the existing
  `dsh`/`opencode`/`llm-mesh-gateway` cases (~line 982-1029) for `"terminal"`: pull a
...
- Update `frameworkStartsWorkspace()` (line 182) to return `true` for the new
  value(s) so the egress-allowlist + socket-guard sidecar (Requirement) attach
  correctly — this is a single-container framework like `dsh`/`opencode`, not
  a multi-container stack like `llm-mesh-gateway`.
...
<<<<<<< HEAD:knowledge/container-and-runtime-security/terminal-workspaces/r23-terminal-tmux-vnc-opencode-deploy.md
- If the VNC-desktop path adopts openmesh-crew's mechanism: replace the current
  bare `ghcr.io/ji-workstation/opencode-vnc:latest` run line (line 1001) with a
  OpenMesh-built image (Dockerfile checked into this repo, so it can actually be
=======
- If the VNC-desktop path adopts llm-mesh-gateway-crew's mechanism: replace the current
  bare `ghcr.io/ji-podhead/opencode-vnc:latest` run line (line 1001) with a
  The Multi-Provider Gateway-built image (Dockerfile checked into this repo, so it can actually be
...
**Frontend changes** (the Workspace Management View):
...
- Reuse the existing iframe embed path (the "Active Framework UI" iframe,
  ...
  page.
- the Authorization Engine:353` (`TypeToCategory`) already has a
