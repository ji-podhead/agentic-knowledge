---
okf_version: "1.0"
id: "okf-con-san-gvisor-architecture"
title: "Basics"
...
[ Host Linux Kernel ]
Summary for dein visuelles Sheet:Sentry zu Gofer: Läuft about Shared Memory (LISFS-Protokoll), um Context Switches zu minimieren.App zu Sentry: Läuft about Systrap-Stubs, die den Thread einfrieren and die Syscall-Daten via Shared Memory an den Sentry-Prozess übergeben.Sicherheitsnetz: Sowohl Sentry als auch Gofer sind with seccomp-bpf-Filtern gegenüber dem Host-Kernel verrammelt. Der Sentry darf fast nur Netzwerk- and Speicher-Syscalls machen, der Gofer fast nur Lese-/Schreib-Syscalls on zugewiesenen Datei-Handles.Damit hast du alle architektonischen Komponenten zusammen, um ein absolut exaktes, professionelles Whitepaper-Diagramm von gVisors Innenleben zu zeichnen.Benötigst du noch spezifische Kennwerte or Bezeichnungen for die Netzwerk-Pfeile (wie z.B. das AF_PACKET-Verhalten), or bist du bereit for das Zeichnen deines Diagramms?
...
hole mir gute quellen and example flow charts and bilde rzu den topicsHier sind exzellente, offizielle Sources, Whitepapers and visuelle Flussdiagramme zu den drei Sicherheitsarchitekturen. Du kannst diese direkt als technische Grundlage (Prior Art) for deine Dokumentation and dein Whitepaper verwenden.1. Offizielle Fachquellen & WhitepapersgVisor Kernel-Isolierung & Netstack:gVisor Architecture Guide (Google / gVisor.dev): Dokumentiert im Detail, wie der integrierte Netstack (Go-Anwendungs-Kernel) rohe Ethernet-Frames liest and den Host-Kernel before direkten Syscalls abschirmt.gVisor Networking Security Blog: Zeigt on, warum gVisor nur bis zu 3 Host-Syscalls benötigt and unvertrauenswürdigen Paketcode isoliert. [1] (https://gvisor.dev/docs/user_guide/networking/), [2] (https://gvisor.dev/docs/architecture_guide/networking/), [3] (https://gvisor.dev/blog/2020/04/02/gvisor-networking-security/)3-Ebenen-Plattform-Kombination (Defense in Depth):Kubernetes Agent Sandbox (SIG Apps): Ein Open-Source-Projekt, das exakt diese 3-Layer-Architektur (gVisor + Netzwerksegmentierung + Identity-basierter Ingress-Router) for AI-Agents and Entwickler-Workspaces standardisiert.Fly.io Architecture Whitepaper: Die Referenz for isoliertes Identity-Proxy-Routing (fly-proxy) in Kombination with harter lokaler Netzwerksegmentierung on Host-Ebene. [1] (https://kubernetes.io/blog/2026/03/20/running-agents-on-kubernetes-with-agent-sandbox/), [2] (https://github.com/kubernetes-sigs/agent-sandbox)2. Visuelles Flussdiagramm: Der Paket-Lebenszyklus(Dieses Flussdiagramm zeigt die Kaskade vom Client-Request bis tief in den sandboxten Go-Kernel von gVisor)text [ NUTZER / CLIENT ]
         │
 ...
 └────────────────────────────────────────────────────────┘
3. Wireshark & Telemetrie-Bilder (Prior Art Nachweis)Wenn du den Traffic with Wireshark or about das Terminal analysierst, kannst du folgende Struktur-Mockups for deine Abbildungen nutzen:Abbildung A: Wireshark Layer-2 VLAN Filter (Beweis for Ebene B)Wenn du tshark or Wireshark on dem Host laufen lässt, filterst du den Traffic der Kunden-Sandboxen about die L2-VLAN-ID. [1] (https://www.researchgate.net/publication/341979907_Design_a_VLAN_Virtual_Local_Area_Network_Based_Network)text$ tshark -i docker0 -Y "vlan.id == 102"
...
Abbildung B: HTTP-Header-Injektion im Sentry (Beweis for Ebene C)Sobald das Paket den Proxy passiert hat, ist die verifizierte App-ID unmanipulierbar im HTTP-Header injiziert, sodass nachgelagerte Tracking-Systeme (wie Prometheus) den Durchsatz zuordnen können:httpGET /api/v1/workspace HTTP/1.1
...
Summary for dein Dokument:Nutze das Kubernetes Agent Sandbox (SIG Apps)-Framework als Beleg, dass die Industrie gVisor gezielt zur Reduzierung der Kernel-Angriffsfläche with dedizierten Routern paart.Das VLAN-Modell stellt sicher, dass selbst at kompromittiertem Proxy no Kunde den Traffic eines anderen belauschen kann. [1] (https://medium.com/@tahirbalarabe2/kubernetes-agent-sandbox-ai-security-d24978512179), [2] (https://kubernetes.io/blog/2026/03/20/running-agents-on-kubernetes-with-agent-sandbox/), [3] (https://www.researchgate.net/publication/341979907_Design_a_VLAN_Virtual_Local_Area_Network_Based_Network)Möchtest du, dass ich dir eine formale Einleitung (Management Summary) for diese 3-Layer-Architektur schreibe, die du direkt in deine Dokumentation einfügen kannst?
