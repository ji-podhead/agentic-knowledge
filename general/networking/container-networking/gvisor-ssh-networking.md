---
okf_version: "1.0"
id: "okf-net-con-gvisor-ssh-networking"
...
summary: "The user requested a massive architectural change to resolve ongoing Docker networking isolation issues (where workspace containers could not reach th"
---

# R29: gVisor Networking, NAT Control, and SSH Agent Forwarding Overhaul

## 1. Executive Summary
The user requested a massive architectural change to resolve ongoing Docker networking isolation issues (where workspace containers could not reach the `llm-mesh-gateway` gateway) and SSH authentication issues (where the backend required direct access to private key files).

The goal is to:
1. **Implement gVisor (`runsc`)** for granular network isolation and NAT control.
2. **Introduce a "Network" Tab** in the UI to let system administrators define network topologies per project.
3. **Migrate to SSH Agent Forwarding** to remove the dangerous `${HOME}/.ssh` volume mount, ensuring private keys never enter the Docker container.

## 2. Database Schema Updates
To support dynamic networking, the `projects` table must be expanded in the Backend Gateway Core:
```sql
...
*   `network_mode`: `isolated` (creates a dedicated bridge), `llm-mesh-gateway-net` (attaches to the gateway's network), `none` (total isolation).
...
**Changes to `docker-compose.yml` (`llm-mesh-gateway` service):**
*   Remove: `- ${HOME}/.ssh:/root/.ssh:ro`
...
1.  **Network Attachment:**
    *   If `network_mode == "llm-mesh-gateway-net"`, append `--network gateway_gateway-mesh-net` to the `docker run` command.
...
4.  **Universal The Multi-Provider Gateway Access:** The `llm-mesh-gateway` gateway container stays securely on `llm-mesh-gateway-net`. It will act as the single proxy/gateway for all projects attached to this network.

## 5. Frontend Updates (the Workspace Management View)
...
*   **Controls:**
    *   Dropdown: Network Mode (Isolated Bridge, The Multi-Provider Gateway Shared Network, Airgapped/None).
