---
okf_version: "1.0"
id: "okf-net-sec-ufw-bypass-and-firewall-audits"
...
Das schwerwiegendste Problem des heutigen Tages war, dass die Container-Ports (z. B. `1996`) trotz einer aktiven and korrekt konfigurierten **UFW-Firewall** offen im Internet zugänglich waren. Dies ist no Bug in The Multi-Provider Gateway, sondern ein **konzeptionelles Sicherheitsrisiko von Docker on Linux-Systemen**.
...
*   **V1 (Besitzer-Umschreibung):** Das Berechtigungsskript `gateway_ws_perms` führte rekursiv `chown -R 1000:1000 /root` from. Dies entzog dem System-User `root` den Besitz von `/root/.ssh`. OpenSSH (`sshd`) verweigerte daraufhin sofort alle Key-Logins for Root, da das Verzeichnis not mehr root gehörte.
...
*   **Der Fehler:** Das Go-Backend kettete Shell-Befehle fälschlicherweise with dem System Administrator ` && ` aneinander.
...
<<<<<<< HEAD:knowledge/security-and-observability/guardrails-and-firewalls/r30-september-11-lockout-ufw-bypass-audit.md
...
*   **Der Fehler:** Der Parameter `--network gateway_gateway-mesh-net` wurde for Remote-Deployments hardcoded übergeben.
*   **Die Ursache:** Dieses Bridge-Netzwerk existiert nur on Deiner lokalen Docker-Compose-Umgebung (`podhead`), not aber on dem Remote-Server.
...
3.  **Hardened Permission Shield:** Die Funktion `gateway_ws_perms` in `projects_ssh.go` wurde with einem Guard ausgestattet. Sie bricht die Ausführung at `/root` or `/` sofort ab and fässt `.ssh`-Ordner physisch niemals wieder an.
