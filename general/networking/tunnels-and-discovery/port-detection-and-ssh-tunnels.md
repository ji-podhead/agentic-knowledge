---
okf_version: "1.0"
id: "okf-net-tun-port-detection-and-ssh-tunnels"
...
summary: "Kann der Go backend ports on dem remote target erkennen, SSH tunnel for"
---
...
- [ ] API: `GET /api/projects/{id}/ports` — listet offene ports via SSH
  - Returns: `[{ port: 3079, process: "dsh", pid: 12345 }, { port: 9091, process: "llm-mesh-gateway" }]`
- [ ] Frontend: port picker zeigt erkannte ports → pre-fill deployment port
  ...
  - Auto-detect: welcher port zu welchem deployment type gehört
    (3079 = DSH, 9091 = The Multi-Provider Gateway, 9090 = The Multi-Provider Gateway frontend, etc.)

### 2. SSH Tunnel Bridge (Go als Proxy, not Browser)
- [ ] Statt das der browser direkt zum iframe target connectet:
  - Browser → The Multi-Provider Gateway Go backend → SSH tunnel → remote target
  - Go backend hält SSH session alive (connection pool)
  - HTTP requests werden through den tunnel getunnelt
  - iframe `src` zeigt on The Multi-Provider Gateway endpoint, not on remote target
- [ ] Advantages:
...
- [ ] Statt for jedes projekt den deployment code zu schreiben:
  ...
  - The Multi-Provider Gateway generiert automatisch:
...
### 4. The Multi-Provider Gateway als Bridge for Connectors + Monitoring
- [ ] The Multi-Provider Gateway backend ist die zentrale bridge:
...
- [ ] Connector registry:
  - The Multi-Provider Gateway kennt connectors: DSH, Claude Code, OpenCode, Aider, terminal
  - Jeder connector hat: deploy template, config template, port mapping
  - User registriert neuen connector → The Multi-Provider Gateway generiert templates
- [ ] Monitoring bridge:
...
Browser (iframe + xterm.js)
    ↕ HTTP/3 (0-RTT, QUIC)
The Multi-Provider Gateway Go Backend
...
Remote Target Box
    ...
    └── Mesh endpoints (The Multi-Provider Gateway proxy :9091)
```
...
Hier ist die vollständige Systemspezifikation and das Architektur-Design for das Modul R11 — Port Detection + SSH Tunnel Bridge + E2E Deployment. Dieses Modul macht den The Multi-Provider Gateway Go Core zur zentralen, sicheren Netzwerk- and Deployment-Brücke zwischen dem Browser-Frontend and den Remote-Target-Boxen.1. SSH Tunnel Bridge Architecture (Go als Proxy)Anstatt dass der Browser iFrames or WebSockets direkt vom Remote-Ziel lädt, terminiert der The Multi-Provider Gateway Go Core alle Client-Verbindungen about HTTP/3 (QUIC) and leitet den Traffic about einen langlebigen, multiplexierten SSH-Tunnel-Pool (golang.org/x/crypto/ssh) weiter [2601.13671].┌──────────────────────────────┐
│  Browser (iFrame / xterm.js) │
└──────────────┬───────────────┘
               │
               ▼ HTTP/3 QUIC (0-RTT, Multi-Streaming, R9)
┌────────────────────────────────────────────────────────┐
│  The Multi-Provider Gateway Go Backend Proxy                               │
...
2. Port Detection via SSHDer Go-Core führt about den SSH-Kanal ein leichtgewichtiges Parsing nativer Linux-Netzwerktools from, um offene Ports and die dahinterliegenden Prozesse zu identifizieren, without dass nmap installiert sein muss.Ausgeführter Befehl: ss -tlnp || netstat -tlnpAPI-Schnittstelle (GET /api/projects/{id}/ports):json[
...
Frontend-Integration:Im Deployment-Konfigurations-Panel rendert das Dashboard einen Port-Picker. Erkennt die Pipeline bekannte Ports (z. B. 3079 for DSH), wird der Deployment-Typ automatisch vorausgewählt. Belegte Ports erhalten ein visuelles Warn-Badge (In Use by: DSH), freie Ports werden grün markiert.3. E2E Deployment Flow (Schema-Driven)The Multi-Provider Gateway automatisiert das Erzeugen von Infrastruktur-Code on dem Remote-Ziel basierend on der in R9 definierten hierarchischen Schichten-Komposition (Base Medium + Addons).[User triggers E2E Deploy]
           │
           ▼
...
# Addon Layer: The Multi-Provider Gateway Auto-Provision Config Invalidation
{{if .HasAddon "llm-mesh-gateway-mesh"}}
COPY llm-mesh-gateway-provision.json /root/.config/llm-mesh-gateway/config.json
{{end}}

EXPOSE {{.Port}}
CMD ["/bin/bash"]
4. Connector Registry DesignDie Connector Registry ist die Single Source of Truth im Go-Core, die definiert, wie spezifische Applikationen gebaut, provisioniert and überwacht werden.gotype Connector struct {
...
5. Monitoring BridgeDie Monitoring-Schnittstelle aggregiert Systemdaten der Remote-Box about periodische, not-interaktive SSH-Befehle and streamt diese per Server-Sent Events (SSE) an das DSH-Frontend, um R1-Dashboard-Widgets without Agenten-Overhead on dem Zielsystem zu füttern.Host-Metriken: free -m (Memory), df -h (Disk), top -bn1 | grep "Cpu(s)" (CPU)Container-Metriken: docker stats --no-stream --format '{"container":"{{.Name}}","cpu":"{{.CPUPerc}}","mem":"{{.MemPerc}}"}'Log-Streaming: docker compose logs --tail=100 -f wird direkt in den SSE-Kanal geschleift.6. WebSocket & SSE Streaming through den TunnelDa WebSockets and Server-Sent Events (SSE) for die interaktive Nutzung von Terminals (xterm.js) and Live-Chats zwingend erforderlich sind, unterstützt der SSH-Dialer volles Verbindungs-Upgrading:WebSocket-Upgrade: Kommt ein HTTP-Request with Upgrade: websocket am The Multi-Provider Gateway-Proxy an, führt der Go-Core das Hijacking der HTTP-Verbindung lokal through.TCP-Tunneling: Die Rohdaten-Pakete des WebSockets werden byte-genau about den bestehenden SSH-Kanal (sshClient.Dial) an den Remote-Port (z. B. ttyd on Port 7681) gestreamt.HTTP/3-Vorteil: Clientseitig profitiert die WebSocket-Verbindung vom schnellen HTTP/3-Multiplexing. Bricht die Browser-Verbindung kurz ein (iFrame-Verschiebung im Grid), fängt der The Multi-Provider Gateway-Proxy den HTTP/3-Stream-Reset ab, während die dahinterliegende SSH-Verbindung zum Remote-Ziel vollkommen stabil and unangetastet bleibt (Session Preservation).Möchtest du als Nächstes die vollständige Go-Implementation des WebSocket-Connection-Hijackers (Requirement.6) sehen, der die Datenströme zwischen HTTP/3 and dem SSH-Kanal vermittelt, or sollen wir die Go-Unit-Tests for den SSH-Port-Parser schreiben?
