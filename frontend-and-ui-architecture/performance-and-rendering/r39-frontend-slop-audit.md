---
okf_version: "1.0"
id: "okf-fro-per-r39-frontend-slop-audit"
...
summary: "**Datum:** 14.9.2026 · **Agent:** TASK-052 (Dokumentier-Lane, nur docs/) · **Basis:** Arbeitsbaum `main` @ `66994f0` + Live-Stack (Backend :19091, Fro"
---

# R39 — Frontend-Slop-Audit + Lücken-Register (TASK-052)

**Datum:** 14.9.2026 · **Agent:** TASK-052 (Dokumentier-Lane, nur docs/) · **Basis:** Arbeitsbaum `main` @ `66994f0` + Live-Stack (Backend :19091, Frontend :19090, authentifiziertes Live-Curl with Admin-Session)

**Methode:** Voll-Lektüre aller 29 Views (`src/frontend/src/views/*.tsx`) gegen die komplette Backend-Route-Inventur (`HandleFunc` about `src/backend/internal/**`), authentifizierte Live-Curl-Checks on jeden verdächtigen Pfad, i18n-Key-Kreuzcheck (alle `t('…')`-Nutzungen vs. `de.json`/`en.json`), Git-Archäologie for jede Planungs-Behauptung (BACKLOG, Sprint-36, TASK-036-Commit ee7055b). Jede Behauptung with Datei:Zeile; negative Funde explizit. **Live-Curl-Beleg for die Zentrale-Aussage:** unauthentifiziert antwortet die Auth-Middleware (`src/backend/internal/auth/auth.go:576-655`) on *jeden* /api-Pfad with 401 — deshalb wurden alle Route-Checks authentifiziert wiederholt.

**WICHTIG — Basis-Klärung for ProjectsPage:** `git status` zeigt +751/−88 uncommittete Zeilen in `ProjectsPage.tsx` (**TASK-051 arbeitet parallel daran**). Alle Befunde unten beziehen sich on den **SHIPPED-Stand (HEAD)**, sofern not ausdrücklich als „051-in-flight (Arbeitsbaum)" markiert. Working-Tree-Zeilen for 051-Themen: `git show HEAD:src/frontend/src/views/ProjectsPage.tsx` (1881 Z.) vs. Arbeitsbaum (2537 Z.).
...
|---|---|---|---|---|
| S1 | **Drei Fetches on nichtexistierende Routen** `/api/networks/{users,nodes,preauthkeys}` | `views/NetworksPage.tsx:29-33` — authentifizierter Live-Curl: **404/404/404** („404 page not found"); Route-Nirgends-Registrierung: `grep HandleFunc` about `src/backend/internal/**` = 0 Treffer for `api/networks` | Headscale-Proxy-Routen im Gateway bauen (GET /api/networks/* → Headscale-API, wie im Kommentar beschrieben) **or** Seite entfernen | **P1** |
| S2 | **Kommentar behauptet Backend, das not existiert**: „Echte Daten … via /api/networks/* Backend-Proxy — Backend liest HEADSCALE_URL + HEADSCALE_API_KEY from .env" | `views/NetworksPage.tsx:9-11` | Kommentar streichen, bis das Backend existiert | P1 |
| S3 | **Falsche Verifizierung im Task-036-Commit**: „… Backend-Proxy … — live 200/200/200" | Commit `ee7055b` (Message); berührt **nur 5 Frontend-Dateien, no Backend-File** (`git show --stat ee7055b`) | Verifikations-Korrektur in Sprint-36/BACKLOG (036-Zeile „Networks/Agents live 200"); Ursache des Schein-200 erklärt: SPA-Fallback (`gateway.go:197` + `gateway.go:72-83`, staticDir-Default `/frontend-dist`, `cmd/server/main.go:125`) liefert index.html+200 statt JSON — genau der Fail-Mode, den `lib/api.ts:103-117` explizit abfängt | P2 (Doku-Wahrheit) |

Positiv (no Fake-Data-Slop): Die Fehler-UI ist ehrlich — `loadError`-Banner statt erfundener Zahlen (`NetworksPage.tsx:37-38,61-63`). Aber: **Sichtbarkeits-Lücke**: `/networks` ist in keiner Nav-Gruppe (`App.tsx:90-93,126-155`) and not im Command-Palette-Seitenindex (`components/command-palette.tsx:108-121`) — nur per Direkt-URL erreichbar.

#### 🔴 ProjectsPage (HEAD 1881 Z.) — Fake-Success ×2 + fabrizierte Log-„Kontrolle"

| # | Fund | Fundstelle | Fix | Prio |
|---|---|---|---|---|
| S4 | **„Successfully connected remote Workspace Node" without Verbindung**: `handleCreateNode` fügt nur ein localStorage-Objekt hinzu — no einziger Netzwerk-/SSH-Call — and toastet trotzdem Erfolg | `HEAD views/ProjectsPage.tsx:443-472`, Toast: `:465` | Ehrlich machen: „Node hinzugefügt (Verbindung wird beim ersten Deploy geprüft)" or echter SSH-Preflight (Muster: `/api/nodes/provision`, das existiert) | **P1** |
| S5 | **Fabrizierte Deploy-Terminal-Logs**: der Deploy selbst ist echt (POST `/api/projects/deploy`, `:532+`), aber die „Terminal build log emulation" (`:322`) schreibt nachträglich klient-seitig eine erfundene Erfolgsstory: „[ssh] Authenticated successfully", „[env] Verified environment dependencies. Docker active." (`:570-571`), „[ok] … 100% success!" (`:584`) — Schritte, die das Frontend not beobachtet hat | `HEAD views/ProjectsPage.tsx:322,570-584` | echtes Server-Log-Streaming (SSE, Muster existiert: `/api/agents/logs/stream`, `analytics.go:41`) or Zeilen als „Executive Summary der Server-Answer" kennzeichnen | **P1** |
| S6 | **„Project settings saved successfully!" without Persistence**: `handleUpdateProject` without Re-Deploy schreibt nur localStorage and toastet Erfolg; es existiert **no** Backend-Route for Project-Updates (Backend hat nur GET-Liste `projects_ssh.go:1816-1852` + DELETE `:1857-1873`; anlegen/updaten läuft ausschließlich about deploy/redeploy) | `HEAD views/ProjectsPage.tsx:240-274` (Toast `:267`), localStorage `:336-365` | PUT/POST `/api/projects` im Backend ergänzen **or** Toast ehrlich on „lokal gespeichert — Redeploy übernimmt" umformulieren; Liste zusätzlich from GET /api/projects hydratisieren (Backend hat die Zeilen in PostgreSQL) | **P1** (Mandat 3: „must round-trip to PostgreSQL") |

Zusatz-Fund (Security, HEAD): SSH-Node-Credentials (sshPassword/sshKey) liegen **im Klartext im localStorage** (`HEAD:22,316,458-459` + Persistence-Effect `:361`) — Umgehung der Vault-Pflicht (Mandat 9). Fix: Nodes-Credentials via `/api/vault/*` referenzieren, localStorage without Secrets halten. **P1.**

### LÜCKE (8 Views)

| View | Lücke | Fundstelle | Fix | Prio |
|---|---|---|---|---|
| **AgentsPage** (183 Z.) | **Sprint-36-WP1-Filter fehlen komplett**: „[x] Frontend: Agents-Tab bekommt Filter-Dropdowns (endpoints, apps, sessions, agents)" — falsch abgehakt; no einzige Filter-Control in der Datei; Commit ee7055b hat AgentsPage **nie** berührt (`git show --stat`) | Behauptung: `docs/sprints/SPRINT-36-LOGGER-SDK-ROUTER-CDC.md:46`; Realität: ganze Datei `views/AgentsPage.tsx` | Filter-Dropdowns bauen (Query-Params on bestehende Endpoints) ODER Haken ehrlich rückgängigen (Mandat 1: retro honesty) | **P1** |
| **LogsPage** (15 Z.) | **SDK-/Audit-Logs-UI fehlt komplett** (041-Welle, TASK-051-Kern): `GET /api/sdk/logs?type=&level=&workspace=&session=&agent=&range=&limit=` ist live 200 (verify), hat **0** Frontend-Verbraucher (grep about src) | Backend-Filterfläche: `internal/sdklogs/sdklogs.go:247-263`; Fehlen: `views/LogsPage.tsx` (nur RealtimeLogs, konsumiert `/api/agents/logs/stream` ✓) | SDK-Logs-Ansicht with Filter-Dropdowns + type=audit/agent/app-Tabs (→TASK-051, in Arbeit) | **P1** |
| **DiscoveryPage** (22 Z.) | **Empty-State-Text ist falsch veraltet**: „Discovery / Debezium ist noch not gebaut" — Container-Discovery existiert and ist live (`GET /api/discovery` → `{"containers":[],"count":0}`, 200; konsumiert von SecurityExposurePage:310). Nicht gebaut ist nur **DB**-Discovery/Debezium-CDC. Zudem: BACKLOG-Zeile „041 … DiscoveryPage 730Z live" ist falsch — die Datei war in ihrer ganzen Git-Historie immer 22 Z. (nur Commit 3d81c07) | UI-Text: `de.json` `admin.discoveryEmptyTitle/Description`; falscher Kommentar: `App.tsx:118-124`; falsche BACKLOG-Zeile: `docs/backlog/BACKLOG.md:9` | Empty-State on den wahren Scope (DB/CDC) umformulieren; BACKLOG:9 korrigieren; DB-Discovery-UI = eigene 041-Aufgabe | **P2** |
...
| **SecurityExposurePage** (1086 Z.) | (a) **E-Priority Rule-Management-UI fehlt**: FirewallStatusTile ist read-only Capabilities/Version/Tabellen (`:574-678`) — der User sieht seine *manage*dten Rules not („der User SIEHT seine Rules", TASK-050). (b) i18n-Lücke: Hierarchy-Selector komplett hartcodiert EN | (a) `:574-678`; (b) `:320-365` („Project:", „All Projects", „Endpoint:" …) | Rules-Ansicht (read-only Liste der effektiven Rules) ergänzen; Selector i18n'n | (a) **P1** (⚡E-Priority) · (b) P3 |
...
| **Negativ-Fund (no Dead Code):** `RealtimeMonitor.tsx` (11.5k, Sep 3) + `RoutingAnalytics.tsx` (12k, Sep 3) | — | verdrahtet in `App.tsx:35-36` + Routen `App.tsx:519-520` — **OK**, bestätigt die 13.9.-Vorfunde. |
...
| **Sprint-36-WP1-Frontend-Zeile** (`SPRINT-36:46`): Agents-Tab-Filter-Dropdowns, „[x]" | Filter-UI | **not gebaut**, Haken falsch (ee7055b hat AgentsPage nie berührt) | ❌ falsch abgehakt |
...
| **E-Projects-Network-Editing-Tab** (TASK-049, BACKLOG:483): Bridges deployen, Alias→Netz-Zuordnung, Container-Neustart at Netz-Änderung | Editing-Tab | **fehlt komplett** — NetworksPage ist read-only + 404; `/api/router/config` (Router-Alias-Verwaltung, TASK-033) hat **0** Frontend-Verbraucher | ❌ |
...
| **E-Firewall** (BACKLOG:23: WAF-Container pro Deployment, NAT-Node am Edge) | Firewall-UI-Fläche | WAF-Integration noch not begonnen (BACKLOG:19 „Plan fertig, NICHT begonnen"); Frontend-Fläche not existent — konsistent with Plan | ⬜ geplant |
...
**Theme-Breaks (without dark:-Variante):** `FallbackPage:451,585,733,798,819,856` (rose/emerald); `NetworksPage:94` (bg-zinc-400 Offline-Punkt). Positiv: SecurityReportPage benutzt konsequent `dark:`-Varianten (`:108-112`).
...
1. **P0 — 051-Commit-Blocker:** Klartext-SSH-Credentials from dem Arbeitsbaum entfernen (`ProjectsPage.tsx` wt:388-390) — sonst not mergen (Mandat 9).
2. **P1 — NetworksPage backenden or entfernen:** `/api/networks/{users,nodes,preauthkeys}` als echte Headscale-Proxy-Routen bauen (Kommentar 9-11 einlösen) and `/networks` in Nav+Palette verdrahten — aktuell 404-end-to-end (live bewiesen) and unsichtbar.
3. **P1 — SDK-/Audit-Logs-UI (Sprint-36-Kern, 051):** GET `/api/sdk/logs` with type/level/workspace/session/agent-Filtern + audit/agent/app-Tabs in LogsPage/AgentsView; daneben `/api/analytics/by-workspace`.
4. **P1 — Sprint-36:46-Haken ehrlich rückgängigen** (Agents-Filter nie gebaut) and BACKLOG:9 korrigieren („DiscoveryPage 730Z live" war nie wahr) — Mandat 1/5.
5. **P1 — ProjectsPage Fake-Success entschlacken:** „Successfully connected" without Verbindung (HEAD:465) and fabrizierte Deploy-Log-Story (HEAD:570-584) through ehrliche Texte/echtes SSE-Log-Streaming ersetzen; SSH-Node-Secrets from localStorage in Vault-Referenz.
...
7. **P1 — E-Priority Rule-Management-UI (TASK-050, ⚡):** FirewallStatusTile um die effektiven Rules des openmesh-Tables erweitern (read-only for den User, verwaltet through Control-Plane) — Fundstelle SecurityExposurePage.tsx:574-678.
...
*(Größere Register-Lücken — E-Projects-Network-Editing-Tab, Vault-RBAC-Verwaltungs-UI, Epic-3-WP4-SSE-Panel, DB-Discovery-UI — sind planbare Features (TASK-049/050, Backlog:356, Backlog:28) and gehören in die Planung, not in die Fix-Welle.)*

---

## 8) Fundstellen-Zählung

- **Slop-Funde: 6** (S1 NetworksPage unbound-Trio · S2 false Backend-Kommentar · S3 false „live 200"-Verifizierung · S4 Fake-„Successfully connected" · S5 fabrizierte Deploy-Logs · S6 „saved" without Persistence) + 1 P0-in-flight (Credentials, uncommittet)
